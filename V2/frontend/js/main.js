document.addEventListener('DOMContentLoaded', function () {
    ...
    // Helper function to check if a trial is a grid search trial
    function isGridSearchTrial(trial) {
    ...
    async function loadInitialData() {
        try {
            appState.participants = await apiRequest('/api/experiments/participants');
            appState.geometries = await apiRequest('/api/experiments/geometries');
            appState.trials = await apiRequest('/api/experiments/trials');
    ...
    systematic.downloadDataBtn.addEventListener('click', async () => {
        ...
        try {
            const response = await apiRequest(`/api/experiments/participants/${participantId}/download`);
    ...
    async function refreshParticipantView(participantId) {
        try {
            const data = await apiRequest(`/api/experiments/participants/${participantId}`);
    ...
    systematic.saveParticipantBtn.addEventListener('click', async () => {
        ...
        try {
            const newParticipant = await apiRequest('/api/experiments/participants', 'POST', payload);
    ...
    systematic.deleteParticipantBtn.addEventListener('click', async () => {
        ...
        if (confirm('Are you sure you want to delete this participant? This action cannot be undone.')) {
            try {
                await apiRequest(`/api/experiments/participants/${participantId}`, 'DELETE');
    ...
    function renderParticipantTrialsTable(participantId) {
        console.log('renderParticipantTrialsTable called with participantId:', participantId);
        console.log('appState.participants:', appState.participants);
        console.log('appState.trials:', appState.trials);
        ...
        console.log('numericParticipantId:', numericParticipantId);
        ...
        console.log('filtered participantTrials:', participantTrials);
    ...
    async function openTrialForEditing(trialId) {
        try {
            const results = await apiRequest(`/api/experiments/trials/${trialId}/details`);
    ...
    async function requestEditRecalculation() {
        ...
        try {
            ...
            const results = await apiRequest('/api/experiments/trials/recalculate', 'POST', payload);
    ...
    async function deleteTrial(trialId, geometryId) {
        try {
            await apiRequest(`/api/experiments/trials/${trialId}`, 'DELETE');
    ...
    systematic.resaveTrialBtn.addEventListener('click', async () => {
        ...
        try {
            await apiRequest(`/api/experiments/trials/${trialId}`, 'PUT', payload);
        ...
        if (trialState.running) {
            ...
            if (trialDataBuffer.length > 0) {
                ...
                try {
                    ...
                    const results = await apiRequest('/api/experiments/trials/analyze', 'POST', payload);
    ...
    systematic.trialForm.addEventListener('submit', async (e) => {
        ...
        try {
            ...
            const newTrial = await apiRequest('/api/experiments/trials/save', 'POST', payload);
    ...
    function renderPainHistory() {
        ...
        painState.trials.forEach((trial, index) => {
            ...
            const sourceIndicator = isGridSearchTrial(trial) ? ' (Grid Search)' : '';
    ...
    async function stopInstabilityDataCollection() {
        try {
            ...
            const response = await apiRequest('/api/experiments/trials/analyze', 'POST', analysisData);
    ...
    async function recalculateInstabilityAnalysis() {
        try {
            ...
            const response = await apiRequest('/api/experiments/trials/recalculate', 'POST', recalcData);
    ...
    function renderInstabilityHistory() {
        ...
        instabilityState.trials.forEach((trial, index) => {
            ...
            // Add source indicator for Grid Search trials
            const trialNumber = isGridSearchTrial(trial) ? `${index + 1} (Grid Search)` : index + 1;

            // Make both Grid Search and Instability BO trials clickable (they have raw data for editing)
            if (isGridSearchTrial(trial) || trial.source === 'instability_bo') {
